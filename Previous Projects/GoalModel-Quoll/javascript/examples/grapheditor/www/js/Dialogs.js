...
var AboutDialog = function(editorUi)
{
	...
	mxUtils.write(h3, mxResources.get('about') + ' Goal Model Editor');
	div.appendChild(h3);
	var img = document.createElement('img');
	img.style.border = '0px';
	img.setAttribute('width', '176');
	img.setAttribute('width', '151');
	img.setAttribute('src', IMAGE_PATH + '/unilogo.png');
	div.appendChild(img);
	mxUtils.br(div);
	mxUtils.br(div);
	mxUtils.write(div, 'Powered by mxGraph ' + mxClient.VERSION);
	mxUtils.br(div);
	mxUtils.write(div, 'Chong Hin Chau, Yuwei Bao, William Pan, Nancy Lee');
	mxUtils.br(div);
	mxUtils.br(div);
...
var EditDiagramDialog = function(editorUi)
{
	...
	var leftDiv = document.createElement('th');
	var rightDiv = document.createElement('th');
	...
	userprompt.innerHTML =
		`
		For each goal, enter in order separated by space: goal type, goal ID, goal name, parent ID
		<br><br>
		f - Functional Goal
		<br>
		e - Emotional Goal
		...
		n - Negative Goal
		<br><br>
		The input from the text box will create the following goal model
		`
	var img = document.createElement('img');
	img.setAttribute("src", "text/defaultModel.png");
	img.setAttribute("height", "180");
	userprompt.appendChild(img);
	leftDiv.appendChild(userprompt);
    ...
    textarea.value = readInputList(editorUi);
    div.appendChild(table);

    /**
	 * Button to generate new xml from an input list
     */
	var btnmodify = mxUtils.button(mxResources.get('ok'), function()
	{
		var values = generateXMLAndRefreshCanvas(textarea.value.toString(), editorUi);

		// update jstree in sidebar
		updateSidebarTree(values[0], values[1], editorUi);
		// update sidebar textarea
		document.getElementById('sidebarTextarea').value = textarea.value.toString();
	});
	btnmodify.className = 'geBtn';
