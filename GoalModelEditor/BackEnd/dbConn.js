...
const SQL_USER_LOGIN =
    "UPDATE User SET LastLogin = NOW()" + "WHERE Username = ? AND Password = ?";
/**
...
const SQL_RET_PROJECTID =
    "SELECT * from Project WHERE ProjectName = ? AND OwnerId = ?";
/**
 * The SQL sentence to create a project
 * @type {string}
 */
const SQL_Project_Creation =
    "INSERT INTO " +
...
const SQL_CREATE_GOALMODEL =
    "INSERT INTO GoalModel (ModelId,ModelName, ModelDescription, URL, ProjectId) " +
    "VALUES (UUID(), ?, ?, ?, ?)";
const SQL_RET_GOALMODEL =
    "SELECT * FROM GoalModel WHERE ModelName = ? AND ProjectId = ?";
/**
...
dbConn.createGoalModel = function(modelName, modelDescription, url, ProjectId) {
    return new Promise(function(resolve, reject) {
        pool.query(
            ...
            [modelName, modelDescription, url, ProjectId],
            function(err, result) {
                if (err) {
                    //console.log(JSON.stringify(err));
                    if (err.errno == 1062) {
