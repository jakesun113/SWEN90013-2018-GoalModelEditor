...
describe("user unit test",function(){
    ...
    it("should sign up successfully",function(done){
        server
            .post('/user/register')
            .send({username : 'zxc', password : 'zxc', firstname : 'xc', lastname : 'z', email : 'zxc@student.unimelb.edu.au'})
            .expect("Content-type",/json/)
            .expect(200)
            .end(function(err,res){
                should(res.status).equal(200);
    ...
    it("should not sign up successfully",function(done){
        server
            .post('/user/register')
            .send({username : 'testfz', password : 'testfz', firstname : 'testfz', lastname : 'testfz', email : 'testfz'})
            .expect("Content-type",/json/)
            .expect(409)
            .end(function(err,res){
