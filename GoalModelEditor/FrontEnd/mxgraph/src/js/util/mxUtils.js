...
var mxUtils = {
        ...
        if (node != null) {
            ...
            if (node.nodeType == mxConstants.NODETYPE_TEXT) {
                ...
                if (tmp != null) {
                    ...
                    while (tmp != null) {
                        result.push(
                            mxUtils.getPrettyXml(tmp, tab, indent + tab)
                        );
                        tmp = tmp.nextSibling;
                    }
    ...
    setTextContent: function(node, text) {
        if (node.innerText !== undefined) {
            ...
            node[
                node.textContent === undefined ? "text" : "textContent"
            ] = text;
        }
    },
    ...
    arcToCurves: function(
        ...
        var rad =
            Math.atan2((ryd - tyd) / r2, (rxd - txd) / r1) - Math.atan2(0, 1);
    ...
    getOffset: function(container, scrollOffset) {
        ...
        if (!scrollOffset && !fixed) {
            var offset = mxUtils.getDocumentScrollOrigin(
    ...
    getViewXml: function(graph, scale, cells, x0, y0) {
        ...
        if (graph.dialect == mxConstants.DIALECT_SVG) {
            ...
            view.overlayPane = document.createElementNS(
                mxConstants.NS_SVG,
                "g"
            );
    ...
    getScaleForPageCount: function(pageCount, graph, pageFormat, border) {
        ...
        var currentTotalPages =
            Math.ceil(numRowPages) * Math.ceil(numColumnPages);
        ...
        while (currentTotalPages > pageCount) {
            ...
            if (roundRowDownProportion == 1) {
                roundRowDownProportion =
                    Math.floor(numRowPages - 1) / numRowPages;
            }
        ...
        if (isInternalWindow) {
            ...
            if (mxClient.IS_NS) {
                ...
                wnd.document.writeln(
                    "<pre>" + mxUtils.htmlEntities(content) + "</pre"
                );
                wnd.document.close();
            } else {
                var wnd = window.open();
