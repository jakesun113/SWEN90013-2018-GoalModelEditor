...
mxVmlCanvas2D.prototype.image = function(
    ...
    if (this.state.alpha < 1 || this.state.fillAlpha < 1) {
        ...
        node.style.filter +=
            "alpha(opacity=" +
            this.state.alpha * this.state.fillAlpha * 100 +
            ")";
    }
...
mxVmlCanvas2D.prototype.createDiv = function(str, align, valign, overflow) {
    ...
    if (
        (state.fontStyle & mxConstants.FONT_ITALIC) ==
    ...
    if (this.textEnabled && str != null) {
        ...
        if (format == "html") {
            ...
            if (this.rotateHtml && rot != 0) {
                ...
                var real_sin = parseFloat(
                    parseFloat(Math.sin(-rad)).toFixed(8)
                );
                ...
                if (abs.nodeName == "group" && this.root.nodeName == "DIV") {
                    ...
                    pos.style.left =
                        this.format(x + (left_fix - w / 2) * s.scale) + "px";
                    pos.style.top =
                        this.format(y + (top_fix - h / 2) * s.scale) + "px";
                    ...
                    abs.style.left =
                        this.format(x + (left_fix - w / 2) * sc) + "px";
                    abs.style.top =
                        this.format(y + (top_fix - h / 2) * sc) + "px";
                }
                ...
                abs.style.left =
                    this.format(x + w * dx * this.state.scale) + "px";
                abs.style.top =
                    this.format(y + h * dy * this.state.scale) + "px";
            } else {
                if (this.state.alpha < 1) {
                    div.style.filter =
                        "alpha(opacity=" + this.state.alpha * 100 + ")";
