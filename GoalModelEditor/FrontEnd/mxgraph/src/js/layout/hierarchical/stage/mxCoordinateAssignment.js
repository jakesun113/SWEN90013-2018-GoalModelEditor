...
mxCoordinateAssignment.prototype.minNode = function(model) {
    ...
    while (nodeList.length > 0 && count <= maxTries) {
        ...
        var nextLayerConnectedCells = cell.getNextLayerConnectedCells(
            rankValue
        );
        var previousLayerConnectedCells = cell.getPreviousLayerConnectedCells(
            rankValue
        );
...
mxCoordinateAssignment.prototype.medianXValue = function(
    ...
    for (var i = 0; i < connectedCells.length; i++) {
        medianValues[i] = connectedCells[i].getGeneralPurposeVariable(
            rankValue
        );
    }
...
mxCoordinateAssignment.prototype.localEdgeProcessing = function(model) {
    ...
    for (var rankIndex = 0; rankIndex < model.ranks.length; rankIndex++) {
        ...
        for (var cellIndex = 0; cellIndex < rank.length; cellIndex++) {
            ...
            if (cell.isVertex()) {
                var currentCells = cell.getPreviousLayerConnectedCells(
                    rankIndex
                );
                ...
                for (var k = 0; k < 2; k++) {
                    if (
                        ...
                        for (var j = 0; j < sortedCells.length; j++) {
                            ...
                            if (innerCell.isVertex()) {
                                ...
                                for (
                                    ...
                                    if (
                                        connections[connIndex].source ==
                                            innerCell ||
                                        connections[connIndex].target ==
                                            innerCell
                                    ) {
                                        connectedEdgeCount +=
                                            connections[connIndex].edges.length;
                        ...
                        if (
                            cell.width >
                            requiredWidth + 2 * this.prefHozEdgeSep
                        ) {
                        ...
                        var currentYOffset =
                            this.minEdgeJetty - this.prefVertEdgeOff;
                        var maxYOffset = 0;

                        for (var j = 0; j < connectedEdges.length; j++) {
                            var numActualEdges = connectedEdges[j].edges.length;
                            var pos = this.jettyPositions[
                                connectedEdges[j].ids[0]
                            ];
    ...
    if (cell.temp[0] != 101207) {
        ...
        for (var i = 0; i < cell.edges.length; i++) {
            ...
            for (
                ...
                var topChannelY =
                    (this.rankTopY[currentRank] +
                        this.rankBottomY[currentRank + 1]) /
                    2.0;
                var bottomChannelY =
                    (this.rankTopY[currentRank - 1] +
                        this.rankBottomY[currentRank]) /
                    2.0;
            ...
            if (jettys != null) {
                ...
                var realTarget = this.layout.getVisibleTerminal(
                    realEdge,
                    false
                );
