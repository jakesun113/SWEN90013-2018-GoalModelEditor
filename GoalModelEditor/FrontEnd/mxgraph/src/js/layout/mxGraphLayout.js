            ...
            if (result == null || result) {
                ...
                if (edgeCount > 0) {
                    for (var i = 0; i < edgeCount; i++) {
                        ...
                        var isSource =
                            this.graph.model.getTerminal(e, true) == vertex;

                        if (!directed || isSource) {
                            var next = this.graph.view.getVisibleTerminal(
                                e,
                                !isSource
                            );
...
mxGraphLayout.prototype.getVertexBounds = function(cell) {
    ...
    if (this.useBoundingBox) {
        ...
        if (
            ...
            var dx1 =
                Math.max(tmp.x + tmp.width - (state.x + state.width), 0) /
                scale;
            var dy1 =
                Math.max(tmp.y + tmp.height - (state.y + state.height), 0) /
                scale;
