    ...
    if (angle != 0) {
        ...
        if (model.isVertex(cell) || model.isEdge(cell)) {
            if (!model.isEdge(cell)) {
                ...
                if (style != null) {
                    var total =
                        (style[mxConstants.STYLE_ROTATION] || 0) + angle;
            ...
            if (geo != null) {
                ...
                if (pgeo != null && !model.isEdge(parent)) {
                    ...
                    geo.rotate(
                        angle,
                        new mxPoint(pgeo.width / 2, pgeo.height / 2)
                    );
                    model.setGeometry(cell, geo);
                }

                if (
                    (model.isVertex(cell) && !geo.relative) ||
                    model.isEdge(cell)
                ) {
                    // Recursive rotation
...
mxVertexHandler.prototype.redrawHandles = function() {
    ...
    if (this.customHandles != null) {
        for (var i = 0; i < this.customHandles.length; i++) {
            ...
            this.customHandles[
                i
            ].shape.node.style.visibility = this.graph.isEditing()
                ? "hidden"
                : "";
        }
    }
    ...
    if (this.selectionBorder != null) {
        if (this.parentHighlight != null) {
            ...
            if (this.graph.model.isVertex(parent)) {
                ...
                if (pstate != null) {
                    this.parentHighlight = this.createParentHighlightShape(
                        pstate
                    );
                    // VML dialect required here for event transparency in IE
...
mxVertexHandler.prototype.drawPreview = function() {
    if (this.preview != null) {
        ...
        if (this.preview.node.parentNode == this.graph.container) {
            this.preview.bounds.width = Math.max(
                0,
